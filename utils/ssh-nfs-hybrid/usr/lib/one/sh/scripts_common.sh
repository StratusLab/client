...
function download_and_extract
{
    ...
    $SSH_CMD $CURL -k -o "/var/lib/one/ebs/cache/$(basename $URL)" $URL
    $SSH_CMD $EXTRACT_CMD "/var/lib/one/ebs/cache/$(basename $URL)"
    log_debug "download in ebs cache complete"
    $SSH_CMD "qemu-img create -f qcow2 -b /var/lib/one/ebs/cache/${$(basename $URL)%.*} ${ARCHIVE%.*}"
    log_debug "made base image"
